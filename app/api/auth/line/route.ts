...
export async function GET(request: Request) {
  ...
  const target = new URL(request.url).searchParams.get('next') === 'admin' ? '/admin' : '/account';
  ...
  response.cookies.set('vd_oauth', seal({ state, nonce, verifier, redirect, target, exp: Date.now() + 600000 }), { ...cookieOptions, maxAge: 600 });
