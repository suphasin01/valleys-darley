...
export default async function Login({ searchParams }: { searchParams: Promise<{ next?: string; error?: string }> }) {
  ...
  if (await member()) redirect(admin ? '/admin' : '/account');
  const enabled = ready();
  return <section className="min-h-screen bg-[#f5f3ef] px-5 pb-20 pt-32">
    <div className="mx-auto grid max-w-5xl overflow-hidden rounded-3xl border border-black/5 bg-white shadow-xl md:grid-cols-2">
      ...
      <div className="flex flex-col justify-center p-8 md:p-12">
        <p className="text-xs uppercase tracking-[0.25em] text-gray-400">{admin ? 'Administration' : 'Welcome back'}</p>
        <h2 className="mt-4 text-3xl font-semibold">{admin ? 'เข้าสู่ระบบแอดมิน' : 'เข้าสู่ระบบสมาชิก'}</h2>
        <p className="mb-8 mt-4 text-sm leading-7 text-gray-500">{admin ? 'ใช้บัญชี LINE ที่ได้รับสิทธิ์ผู้ดูแลระบบ' : 'เข้าสู่ระบบผ่าน LINE ได้เลย โดยไม่ต้องตั้งรหัสผ่านใหม่'}</p>
